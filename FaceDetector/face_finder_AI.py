...
# Loss weights
W1 = 1.0
W2 = 100.0 # make 1.0 in the begining and then 100.0 for final bbox training!
...
def create_mask(targetsBatch):
    ...
    for j in range (0, targetsBatch.shape[0]):   # loop over batch
        ...
        for i in range (0, MAX_NUM_OF_FACES):
            if currentTarget[0 + i*4] != 0 and currentTarget[1 + i*4] != 0 and currentTarget[2 + i*4] != 0 and currentTarget[3 + i*4]:
...
class FacesDataset(Dataset):
    def __init__(self, pathToManifest, pathToImages, imageTransform=None):
        ...
        counter = 0
        with open(pathToManifest, "r") as f:
            while endOfDataSet == False:
                ...
                numOfFaces = int(f.readline().replace("\n", ""))

                boundingBoxes = np.zeros(MAX_NUM_OF_FACES*4)
                for i in range (0, numOfFaces):
                    target = f.readline().replace("\n", "").split()
                    
                    if i < MAX_NUM_OF_FACES:
                ...
                if numOfFaces <= MAX_NUM_OF_FACES:
                    dataset.append((path, boundingBoxes))
                    
                counter += 1

        self.data = np.array(dataset, dtype=object)
        print("Found {} images, added {} images to the dataset".format(counter, self.data.shape[0]))
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(os.path.join(self.pathToImages, imagePath))
        ...
        targets = self.data[idx][1] / IN_IMG_SIZE
        
        return imageAsTensor, torch.Tensor(targets)
...
def train(device, start_epoch, n_epochs, pathToManifest, pathToData, pathToLogs, pathToModel = None):
    ...
    validationSet = DataLoader(FacesDataset(imageTransform=dataTransformations, 
    ...
    optimizer = optim.AdamW(model.parameters(), lr=1e-4)        #optim.SGD(model.parameters(), lr=0.0001)
